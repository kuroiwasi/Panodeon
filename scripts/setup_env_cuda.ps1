...
try {
    ...
    & $Python -m pip install "onnxruntime-gpu[cuda,cudnn]>=1.21"
    if ($LASTEXITCODE -ne 0) { throw "Failed to install onnxruntime-gpu" }
    & $Python -m pip install nvidia-cublas-cu12 nvidia-cudnn-cu12 nvidia-cuda-runtime-cu12
    if ($LASTEXITCODE -ne 0) { throw "Failed to install CUDA runtime packages" }
    Write-Host "CUDA/NVIDIA ONNX Runtime installed in .venv"
}
finally {
    Pop-Location
}
